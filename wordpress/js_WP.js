<script>
/**
 * ARIA SOLUTIONS — INTERATIVIDADE DA LANDING PAGE (WordPress WPCode)
 */

document.addEventListener('DOMContentLoaded', () => {
  // 0. Hero Background Carousel / Slider
  const heroSlides = document.querySelectorAll('.hero-slider-bg .slide-item, .slide-item');
  if (heroSlides.length > 1) {
    let currentSlide = 0;
    
    // Garante que o primeiro slide esteja ativo se nenhum estiver
    let hasActive = false;
  ...
  if (segmentOptions.length > 0 || goalOptions.length > 0) {
    ...
    segmentOptions.forEach(opt => {
      opt.addEventListener('click', () => {
        ...
        selectedSegment = opt.getAttribute('data-value') || selectedSegment;
        updateSimulation();
      });
    });
    ...
    goalOptions.forEach(opt => {
      opt.addEventListener('click', () => {
        ...
        opt.classList.add('selected');
        selectedGoal = opt.getAttribute('data-value') || selectedGoal;
        updateSimulation();
      });
    });
